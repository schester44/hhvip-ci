var UKEventType = {
 ...
 function modifValues(){
    ...
    var val = $('.uploading-progress-bar').attr('data-value');
    if(val>=100){val=5;}     
  }
...
var UploadKit = function(input) {
  ...
  var uploader_params = $input.data('uploader_params');
  ...
  uploader_params.container = 'uk-container-' + id;
  uploader_params.drop_element = 'uk-container-' + id;
  uploader_params.browse_button = 'uk-browse-button-' + id;
  uploader_params.multi_selection = isMultiple;

  var uploader = this.uploader = new plupload.Uploader(uploader_params);
  ...
  uploader.bind('UploadProgress', function(uploader, file) {    
    ...
    var txt = Math.floor(file.percent)+'%';  
    $('.uploading-progress-bar').attr('data-value',file.percent);
    $('.mixtape-upload-percentage').html(txt);
    $('.uploading-progress-bar').css("width", txt);
  ...
  $uploadButton.bind('click', function(evt) {
    //show the progressbar && hide overwrite warning
    $("#mixtape-overwrite").hide();
    $('#mixtapeProgressBar').show();
