...
class Ion_auth
{
	...
	public function register($username, $password, $email, $additional_data = array(), $group_ids = array()) //need to test email activation
	{
		...
		if (!$email_activation)
		{
			...
			if ($id !== FALSE)
			{
				...
				//auto login and redirect to their profile
	 ...
	 * @author HHVIP
	 **/
	public function is_moderator($id=false)
	{
		$this->ion_auth_model->trigger_events('is_moderator');

		$admin_group = $this->config->item('moderator_group', 'ion_auth');
