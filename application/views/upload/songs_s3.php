...
$(document).ready(function(){
  ...
  if (str.indexOf(schttpurl) >= 0 || str.indexOf(schttpsurl) >= 0) {
	...
	$("#uploader").pluploadQueue({
      ...
      multipart_params : {
        'key': '<?php echo $this->ion_auth->user()->row()->username . "/"; ?>${filename}',
        'Filename': '<?php echo $this->ion_auth->user()->row()->username . "/"; ?>${filename}',
  ...
  uploader.bind('UploadComplete', function(){
    ...
    $.ajax({
			cache: false,
			dataType: 'json',
			data: {id: file.id, response: info.response, name: file.name, size: file.size},
  ...
  $('#submit-btn').click(function(e){
      ...
      $('.confirm-delete-btn').click(function(e){
        ...
        $.ajax({
          cache: false,
          dataType: 'json',
          data: {user: '<?php echo $user->id; ?>', id: data.song_id},
          url: 'manage/song/delete',
          type: 'POST',
          success: function(res) {
            if (res.error === 'true') {
                ...
                $('#deleteformErrors').html(res.message);
            };
          }// success
        });//ajax
    });//confirm
