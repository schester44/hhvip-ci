<div class="ribbon ribbon-highlight">
    <ol class="breadcrumb ribbon-inner">
        ...
        <li><?php echo ucfirst($this->uri->segment('3')); ?> Articles</li>
    </ol>
</div>
...
<div id="content" class="content section row">
    ...
    <?php if (isset($posts) && !empty($posts)): ?>
        <?php foreach ($posts as $key => $post): ?>
           ...
           <a href="<?php echo base_url('b/'.$post->category_title.'/'.$post->url); ?>" title="<?php echo htmlspecialchars($post->title, ENT_QUOTES); ?>"> <img src="<?php echo blog_featured_img($post->username, $post->url, $post->featured_image, 150); ?>"></a>
            ...
            <span style="color:#ccc;font-size:10px;">Posted on <?php echo date('M d, Y', $post->date_published); ?> (<?php echo time_ago($post->date_published); ?>) by <a href="<?php echo base_url('u/'.$post->username); ?>" title="View <?php echo $post->username; ?>'s profile"><?php echo $post->username; ?></a> | Category: <?php echo ucfirst($post->category_title); ?></span>
    ...
    <?php endforeach ?>
    <?php else: ?>  
   <h2 style="text-align:center;"> No more stories</h2>
    <?php endif ?>
